...
# Workspace cargo config — needed for Android target linker rustflags.
...
# Vendored crates required for a `[patch.crates-io]` workspace build.
# Without these, the public mirror does not compile because Cargo
# cannot resolve the patched mcp2221-hal entry in Cargo.toml.
VENDOR_DIR="vendor"

# mdBook sources (NL + EN guides published to GitHub Pages). The
# `book` directory is gitignored and regenerated by `mdbook build`,
# so we only stage `book.toml`, `src/`, `custom.css`, `mermaid-*.js`
# and the PowerShell build/sync/export helpers.
DOCS_BOOK_FILES=(
    docs-book/book.toml
...
# Vendored crates — copied verbatim including the upstream license
# files (LICENSE-MIT, LICENSE-APACHE) so attribution stays intact.
if [ ! -d "$REPO_ROOT/$VENDOR_DIR" ]; then
    fail_with "missing vendor dir: $VENDOR_DIR"
fi
mkdir -p "$STAGING/$VENDOR_DIR"
ex_args=()
for e in "${SOURCE_EXCLUDES[@]}"; do ex_args+=(--exclude="$e"); done
(cd "$REPO_ROOT/$VENDOR_DIR" && tar -c "${ex_args[@]}" .) | (cd "$STAGING/$VENDOR_DIR" && tar -x)

# mdBook sources for the public docs site.
mkdir -p "$STAGING/docs-book/src"
for f in "${DOCS_BOOK_FILES[@]}"; do
    if [ ! -f "$REPO_ROOT/$f" ]; then
        fail_with "missing docs-book file: $f"
    fi
    cp "$REPO_ROOT/$f" "$STAGING/$f"
done
# Copy docs-book/src/* (NL + EN guides). The .md sources are
# regenerated by docs-book/sync-docs.ps1 from the root *.md files;
# we stage the resulting in-tree copy.
cp "$REPO_ROOT/docs-book/src/"*.md "$STAGING/docs-book/src/"
# docs-book/.gitignore excludes the generated book/ output from git.
if [ -f "$REPO_ROOT/docs-book/.gitignore" ]; then
    cp "$REPO_ROOT/docs-book/.gitignore" "$STAGING/docs-book/.gitignore"
fi

# Public-facing docs (top-level only — docs/internal/** is forbidden
# and is blocked by the FORBIDDEN_PATHS scan below).
if [ -d "$REPO_ROOT/docs" ]; then
    mkdir -p "$STAGING/docs"
    # Only top-level files, never subdirectories (internal lives there).
    find "$REPO_ROOT/docs" -maxdepth 1 -type f -print0 \
        | xargs -0 -I{} cp {} "$STAGING/docs/"
fi

# Workspace cargo config (Android target linker rustflags).
if [ -f "$REPO_ROOT/$WORKSPACE_CARGO_CONFIG" ]; then
    mkdir -p "$STAGING/$(dirname "$WORKSPACE_CARGO_CONFIG")"
    cp "$REPO_ROOT/$WORKSPACE_CARGO_CONFIG" "$STAGING/$WORKSPACE_CARGO_CONFIG"
fi

# scripts/ — public subset only (sync + ui-coverage + evidence-scan).
# These were public in v2.0.4; keep parity. scripts/README.md is also
# shipped after the AI-jargon scrub in the README itself.
SCRIPTS_FILES=(
    scripts/sync-to-public.sh
    scripts/check-ui-coverage.sh
    scripts/evidence-scan.sh
    scripts/ui-coverage-expected.json
    scripts/README.md
)
...
FORBIDDEN_PATHS=(
    ...
    Faseplan.md
    RELEASE.md
    # docs/internal/** is the canonical private root (patch-briefs,
    # AI-workflow, audit, planning, release reviews). The two
    # historical entries below stay as defense-in-depth in case a
    # rename slips by.
    docs/internal
    docs/patch-briefs
    docs/audit
