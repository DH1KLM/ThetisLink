...
impl MCP2221 {
    ...
    /// Connect to a specific USB device by USB serial number (default VID/PID).
    ///
    /// Use this when multiple MCP2221 boards are plugged into the same host and
    /// you need to target a particular one. Each board can be assigned a unique
    /// USB serial via [`MCP2221::usb_change_serial_number`]; out of the box the
    /// Microchip default serial is often identical across boards, in which case
    /// you should rename one of them first.
    ///
    /// # Errors
    ///
    /// An error is returned if no MCP2221 with the given serial is connected.
    ///
    /// # ThetisLink fork addition
    ///
    /// Upstream `mcp2221-hal` 0.1.0 marks this as a TODO. This method is added
    /// in the vendored fork so the ThetisLink server can drive two StockCorner
    /// tuners with one MCP2221A per tuner.
    pub fn connect_with_serial(serial: &str) -> Result<Self, Error> {
        Self::connect_with_serial_and_vid_pid(MICROCHIP_VID, MCP2221_PID, serial)
    }

    /// Connect to a specific MCP2221 by USB VID, PID and serial number.
    ///
    /// Convenience for boards whose VID/PID has also been re-flashed away from
    /// the Microchip defaults.
    ///
    /// # ThetisLink fork addition
    ///
    /// See [`MCP2221::connect_with_serial`].
    pub fn connect_with_serial_and_vid_pid(
        vendor_id: u16,
        product_id: u16,
        serial: &str,
    ) -> Result<Self, Error> {
        let hidapi = HidApi::new()?;
        let device = hidapi.open_serial(vendor_id, product_id, serial)?;
        Ok(Self {
            inner: device,
            pins_taken: Cell::new(false),
        })
    ...
    /// Enumerate the currently-connected MCP2221 boards on the default VID/PID.
    ///
    /// Returns one [`Mcp2221Listing`] per board, including the USB serial number
    /// (suitable as input to [`MCP2221::connect_with_serial`]) and the product
    /// string. Boards without a serial number show an empty string — re-flash
    /// them with [`MCP2221::usb_change_serial_number`] before targeting.
    ///
    /// # ThetisLink fork addition
    pub fn list_devices() -> Result<Vec<Mcp2221Listing>, Error> {
        let hidapi = HidApi::new()?;
        let listings: Vec<Mcp2221Listing> = hidapi
            .device_list()
            .filter(|d| d.vendor_id() == MICROCHIP_VID && d.product_id() == MCP2221_PID)
            .map(|d| Mcp2221Listing {
    ...
    /// Open a specific MCP2221 by its OS-level HID device path. The path is
    /// what [`MCP2221::list_devices`] returns in [`Mcp2221Listing::path`];
    /// pre-programming a USB serial is therefore not required to address a
    /// specific board.
    ///
    /// # Errors
    ///
    /// Returns an error if the path does not correspond to an open-able HID
    /// device, or if the path string is not valid UTF-8.
    ///
    /// # ThetisLink fork addition
    pub fn connect_with_path(path: &str) -> Result<Self, Error> {
        use std::ffi::CString;
        // Path strings always come from `hidapi::DeviceInfo::path()` which is a
        // valid C string by construction — but guard with a HidApi error in the
        // unlikely case a caller hands us a string with an interior NUL.
        let cpath = CString::new(path).map_err(|_| {
            Error::HidApi(hidapi::HidError::HidApiError {
                message: "path string contained interior NUL byte".to_string(),
            })
        })?;
        let hidapi = HidApi::new()?;
        let device = hidapi.open_path(cpath.as_c_str())?;
        Ok(Self {
            inner: device,
            pins_taken: Cell::new(false),
        })
...
/// Lightweight description of an MCP2221 USB device, returned by
/// [`MCP2221::list_devices`]. Carries the fields a host normally needs to
/// pick a specific board out of several attached to the same machine.
///
/// # ThetisLink fork addition
///
/// Added so ThetisLink can present a serial-number picker in the server UI.
#[derive(Debug, Clone)]
pub struct Mcp2221Listing {
    /// USB vendor ID reported by the board (default Microchip 0x4D8 = 1240).
    pub vendor_id: u16,
    /// USB product ID reported by the board (default MCP2221 0xDD = 221).
    pub product_id: u16,
    /// USB serial number string, possibly empty if the board has no serial set.
    pub serial_number: String,
    /// USB product descriptor string (defaults to "MCP2221 USB-I2C/UART Combo").
    pub product_string: String,
    /// OS-level HID device path — unique even when the serial number is
    /// empty. Pass to [`MCP2221::connect_with_path`] to target a specific
    /// anonymous board (e.g. for one-shot serial-number programming).
    pub path: String,
}
